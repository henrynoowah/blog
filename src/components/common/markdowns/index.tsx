...
const markdownComponents = {
  h1: ({ node, ...props }: any) => (
    <h1
      className="text-primary dark:text-light font-bold text-2xl py-6"
      {...props}
    >
      {props.children}
    </h1>
  ...
  h2: ({ node, ...props }: any) => (
    <h2
      className="text-primary dark:text-light font-bold text-xl py-6"
      {...props}
    >
      {props.children}
    </h2>
  ...
  h3: ({ node, ...props }: any) => (
    <h3
      className="text-primary dark:text-light font-bold text-lg py-6"
      {...props}
    />
  ),
  h4: ({ node, ...props }: any) => (
    <h4 className="text-primary dark:text-light font-bold text-md" {...props} />
  ),
  h5: ({ node, ...props }: any) => (
    <h5 className="text-primary dark:text-light font-bold text-md" {...props} />
  ),
  h6: ({ node, ...props }: any) => (
    <h5 className="text-primary/60 font-bold text-md" {...props} />
  ),
  p: ({ node, ...props }: any) => (
    <p
  ...
  ul: ({ node, ...props }: any) => {
    ...
    return (
      <ul className="list-disc text-md leading-6 ps-4 md:ps-8" {...rest} />
    );
  },
  ol: ({ node, ...props }: any) => (
    <ol className="list-decimal ps-4 text-md leading-8" {...props} />
  ),
  li: ({ node, ...props }: any) => {
    const { ordered: _, ...rest } = props;
    return (
  ...
  hr: ({ node, ...props }: any) => (
    <hr className="my-6 border-primary/60" {...props} />
  ),
  a: ({ node, ...props }: any) => (
    <a
      target="_blank"
      className="text-primary dark:text-secondary underline underline-offset-1"
      {...props}
    />
  ),
  ...
  img: ({ node, className, children, ...props }: any) => (
    ...
    <img
      {...props}
      alt=""
      className="w-full overflow-hidden rounded-md ring-1 ring-primary/60 shadow-md my-4"
    />
  ),
};
